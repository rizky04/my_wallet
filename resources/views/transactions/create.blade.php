...
<div class="app-bar-plain">
    ...
    <h1 style="font-size:17px;font-weight:600;color:#fff;margin:0">Tambah Transaksi</h1>
</div>

<div class="px" style="padding-top:8px">
    <form method="POST" action="{{ route('transactions.store') }}" style="display:flex;flex-direction:column;gap:20px">
        @csrf
        ...
        <div style="display:flex;background:var(--card);border:0.5px solid var(--border);border-radius:16px;overflow:hidden">
            <label style="flex:1;cursor:pointer">
                ...
                <div id="tab-exp" onclick="setType('expense')" style="padding:14px;text-align:center;font-weight:600;font-size:14px;transition:all 0.15s;{{ old('type','expense')==='expense'?'background:rgba(255,69,58,0.15);color:var(--error)':'color:var(--t2)' }}">
                    Pengeluaran
                </div>
            </label>
            <label style="flex:1;cursor:pointer">
                <input type="radio" name="type" value="income" style="display:none" {{ old('type')==='income'?'checked':'' }}>
                <div id="tab-inc" onclick="setType('income')" style="padding:14px;text-align:center;font-weight:600;font-size:14px;transition:all 0.15s;{{ old('type')==='income'?'background:rgba(48,209,88,0.15);color:var(--success)':'color:var(--t2)' }}">
                    Pemasukan
                </div>
            </label>
        </div>
        ...
        <div>
            ...
            <textarea name="note" class="input-field" rows="2" placeholder="Tambahkan catatan...">{{ old('note') }}</textarea>
        </div>
