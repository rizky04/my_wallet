...
@section('content')
    <h2 style="font-size:20px;font-weight:700;color:#fff;margin:0 0 24px">Buat akun baru</h2>
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
        ...
        <span style="font-size:12px;color:var(--t3)">atau daftar dengan email</span>
        <div style="flex:1;height:0.5px;background:var(--border)"></div>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" style="display:flex;flex-direction:column;gap:16px">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}"
                class="input-field" placeholder="Nama Anda" required autofocus autocomplete="name">
        </div>
        <div>
            <label class="label" for="email">Email</label>
            <input type="email" id="email" name="email" value="{{ old('email') }}"
                class="input-field" placeholder="email@contoh.com" required autocomplete="email">
        </div>
        <div>
            <label class="label" for="password">Password</label>
            <input type="password" id="password" name="password"
                class="input-field" placeholder="Minimal 6 karakter" required autocomplete="new-password">
        </div>
        <div>
        ...
        <button type="submit" class="btn-primary" style="margin-top:4px">Daftar</button>
    </form>
