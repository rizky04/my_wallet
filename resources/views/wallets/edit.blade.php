...
<div class="app-bar-plain">
    ...
    <h1 style="font-size:17px;font-weight:600;color:#fff;margin:0">Edit Dompet</h1>
</div>

<div class="px" style="padding-top:8px">
    <form method="POST" action="{{ route('wallets.update', $wallet) }}" style="display:flex;flex-direction:column;gap:20px">
        @csrf @method('PUT')
        <div>
            ...
            <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px">
                @foreach($icons as $icon)
                    <label style="cursor:pointer">
                        <input type="radio" name="icon" value="{{ $icon }}" style="display:none" {{ old('icon',$wallet->icon)===$icon?'checked':'' }}>
                        <div onclick="selectIcon(this,'{{ $icon }}')" data-icon="{{ $icon }}"
                            style="padding:12px;border-radius:14px;background:var(--card);border:1px solid {{ old('icon',$wallet->icon)===$icon?'var(--primary)':'var(--border)' }};display:flex;align-items:center;justify-content:center;transition:border 0.15s">
        ...
        <div>
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap">
                @foreach($colors as $color)
                    <label style="cursor:pointer">
                        <input type="radio" name="color" value="{{ $color }}" style="display:none" {{ old('color',$wallet->color)===$color?'checked':'' }}>
                        <div onclick="selectColor(this,'{{ $color }}')" data-color="{{ $color }}"
                            style="width:38px;height:38px;border-radius:50%;background:{{ $color }};border:2px solid {{ old('color',$wallet->color)===$color?'#fff':'transparent' }};transition:all 0.15s;transform:scale({{ old('color',$wallet->color)===$color?'1.15':'1' }})">
                        </div>
                    </label>
                @endforeach
            </div>
